...
import { ShoppingBag, User, LogOut, ShieldCheck, Zap } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, onOpenAdmin, onOpenCredits }) => {
  return (
      ...
      <header className="sticky top-0 z-50 glass border-b border-slate-200/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">MATCH</h1>
          </div>

          <nav className="hidden md:flex items-center space-x-8 text-sm font-medium text-slate-600">
            <a href="#" className="hover:text-indigo-600 transition-colors">COLLECTIONS</a>
            <a href="#" className="hover:text-indigo-600 transition-colors">ABOUT</a>
            <a href="#" className="hover:text-indigo-600 transition-colors">CONCIERGE</a>
          </nav>
          ...
          <div className="flex items-center space-x-4">
            {user && (
              <button 
                ...
                className="flex items-center space-x-2 bg-indigo-50 text-indigo-700 px-4 py-2 rounded-full text-sm font-semibold hover:bg-indigo-100 transition-all border border-indigo-100"
              >
            ...
            {user ? (
              ...
              <button className="text-sm font-semibold text-slate-900 border-2 border-slate-900 px-6 py-2 rounded-full hover:bg-slate-900 hover:text-white transition-all">
                SIGN IN
              </button>
            )}
          </div>
